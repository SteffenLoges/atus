...
func (v *ProbedVideo) Extract(s *Stream, newFile string) ([]byte, error) {
	...
	case "video":
		...
		return _exec("ffmpeg", "-y", "-hide_banner", "-loglevel", "warning", "-i", v.File, "-c:v", "libx264", "-an", "-map", fmt.Sprintf("0:%d", s.Index), newFile)
...
func (v *ProbedVideo) ExtractFrame(timestamp, newFile string) error {
	_, err := _exec("ffmpeg", "-y", "-hide_banner", "-loglevel", "warning", "-i", v.File, "-ss", timestamp, "-frames:v", "1", "-update", "1", "-q:v", "2", newFile)
	return err
}
